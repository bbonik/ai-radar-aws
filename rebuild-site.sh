...
if [ "$RUN_PIPELINE" = true ]; then
    ...
    aws lambda invoke --function-name ai-radar-report-pipeline --invocation-type Event /dev/null --region "$REGION" > /dev/null 2>&1
    ...
    aws lambda invoke --function-name ai-radar-website-builder --invocation-type Event --payload fileb:///tmp/_rebuild_payload.json /dev/null --region "$REGION" > /dev/null 2>&1
...
# ─── Show website URL ────────────────────────────────────────────────────────
WEBSITE_URL=$(aws cloudformation describe-stacks --stack-name AiRadarAwsStack --query 'Stacks[0].Outputs[?OutputKey==`WebsiteUrl`].OutputValue' --output text --region "$REGION" 2>/dev/null)
